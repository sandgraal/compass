 ...
 * at runtime) and pick UTC instants that fall on a *different* calendar
 * day/month than the local one — so a `toISOString().slice(...)` regression
 * would fail.
 */

import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import { isoDate, isoMonth, todayISO } from './utils'
...
describe('isoDate', () => {
  it('returns the local day east of UTC when the UTC day is still "yesterday"', () => {
    ...
    expect(isoDate(new Date('2026-05-22T20:00:00.000Z'))).toBe('2026-05-23')
  })
  ...
  it('returns the local day west of UTC when the UTC day is already "tomorrow"', () => {
    ...
    expect(isoDate(new Date('2026-05-23T04:00:00.000Z'))).toBe('2026-05-22')
  })
...
describe('isoMonth', () => {
  it('returns the local month when the UTC instant rolls into the next month locally', () => {
    ...
    expect(isoMonth(new Date('2026-05-31T16:00:00.000Z'))).toBe('2026-06')
  })
  ...
  it('returns the local month when the UTC instant is still the previous month locally', () => {
    ...
    expect(isoMonth(new Date('2026-06-01T04:00:00.000Z'))).toBe('2026-05')
  })
})
