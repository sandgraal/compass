...
export default function Monthly(): JSX.Element {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      // Finance snapshot
      const d = debtData as {
        debts: { id: number; name: string; balance: number | null; apr: number | null }[]
      }
            ...
            <div className="grid grid-cols-7 text-center gap-y-1">
              {calDays.map((day) => {
                ...
                const hasEvent = events.some(
                  (e) => e.startAt && isSameDay(new Date(e.startAt), day)
                )
                return (
                  <div
                    key={isoDate(day)}
                  ...
                  <div className="space-y-1.5">
                    {debtSummary.map((d) => (
                      <div key={d.id} className="flex items-center justify-between">
                        ...
                        <span className="text-xs font-mono text-red-400 shrink-0">
                          $
                          {Math.abs(d.balance ?? 0).toLocaleString('en-US', {
                    ...
                    <div className="flex items-center justify-between border-t border-border pt-1.5 mt-1">
                      ...
                      <span className="text-xs font-mono font-semibold text-red-400">
                        $
                        {debtSummary
                          .reduce((s, d) => s + Math.abs(d.balance ?? 0), 0)
                          ...
                          {l.budget > 0 && (
                            <div className="h-1 bg-secondary rounded-full overflow-hidden">
                              <div
                                ...
                                style={{
                                  width: `${Math.min(100, (Math.abs(l.actual) / l.budget) * 100)}%`
                                }}
                              />
                            </div>
                          )}
            ...
            {habits.length === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr>
                      <th className="text-left text-muted-foreground font-medium pb-2 pr-3 w-32">
                        Habit
                      </th>
                      {daysInMonth.map((d) => (
                        <th
