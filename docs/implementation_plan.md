...
| **Phase 7** — Daily-Driver & Platform Roadmap | 6 tracks | **Scheduled (2026-06-06).** Track A in progress: **Morning Brief** (digest card + scheduled notification), the **weekly review close-out** (`weekly-review:get` + carry-over), and the **monthly rollup** (`monthly-rollup:get`), and the **Morning Brief low-cash alert** (`computeLowCashAlert` over the cash-flow forecast) shipped; remaining Track A (global+voice quick-capture; Morning Brief price-hike subscription alerts) + Tracks B–F still open. See § Phase 7. |
