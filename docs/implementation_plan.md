...
- [x] **P1** — `electron/ipc/finance.ts` (largest handler, 33 handlers) — chunk 1 CRUD shipped via #102 (`finance.test.ts`); chunk 2 business-logic queries shipped 2026-06-05 (`finance-queries.test.ts`, 15 tests: get-debt-summary avalanche, get-geo-summary, get-tax-summary, get-upcoming-payments, get-budget-status). Chunk 3 (file-I/O + watcher: ingest-folder, export-tax-pack, capture-snapshot, set-account-balance, reapply-rules, watch-folder controls) still pending. Thin pass-through query handlers (net-worth, get-subscriptions, get-forecast) are covered by their underlying modules.
