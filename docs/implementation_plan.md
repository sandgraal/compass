...
| **Phase 7** — Daily-Driver & Platform Roadmap | 6 tracks | **Scheduled (2026-06-06).** Track A in progress: **Morning Brief** (digest card + scheduled notification) and the **weekly review close-out** (`weekly-review:get` + carry-over) shipped; remaining Track A (global+voice quick-capture; Morning Brief alerts + monthly rollup follow-ups) + Tracks B–F still open. See § Phase 7. |
...
- [~] **Evening / weekly review ritual** (M) — **weekly close-out shipped 2026-06-06.** Wins/blockers/next reflection + week-over-week completion delta already existed; this added the data side (`electron/ipc/weekly-review.ts`, 100% covered): `weekly-review:get` (completion %, prior-week delta, Mon..Sun per-day breakdown, carry-over candidates) + `weekly-review:carry-over` (copy unfinished **manual** tasks forward to today, idempotent), surfaced as a "carry N unfinished → today" action on the Weekly page. *Remaining: the monthly rollup (aggregate the weeks' completion/reflections on the Monthly page).* *Why: habitual return cadence.*
