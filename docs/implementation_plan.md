...
- [ ] **8.3 Claude Desktop connector** — package `compass-mcp` as a one-click DXT/`.mcpb` desktop-extension bundle + documented `claude_desktop_config.json` fallback. *(Manual config documented in `claude-plugin/README.md`; the bundled artifact is the remaining work — gated on bundling the `better-sqlite3` native dep.)*
