...
- [~] **10.2 Financial & credit completeness** 🟡 *partial — credit-report + tax-doc PDF recognizers + a generic **brokerage-holdings CSV importer** shipped; LIVE holdings (SnapTrade/Plaid Investments), IRS-portal transcripts, crypto open (feeds Phase 11.4)* — credit reports (AnnualCreditReport / FCRA); **brokerage/retirement holdings — FILE path shipped:** a generic positions-CSV recognizer (`electron/integrations/finance-holdings.ts` — matches common Symbol/Quantity/Price/Market-Value/Cost-basis columns across brokers, derives value from qty×price, reads parens-as-negative) stored as dated `records` snapshots (`finance:import-holdings` / `finance:get-holdings`), surfaced as a Net Worth **holdings card** (market value / cost basis / unrealized gain). Column-matching is best-effort and unvalidated against a specific real export — drop a real positions CSV to sharpen it. LIVE auto-feed (SnapTrade or Plaid Investments), IRS/tax transcripts, crypto still open. Extends Phase 4.
