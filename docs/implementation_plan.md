...
| **Phase 8** — Claude Integration (bidirectional) | 6 items | **8.1, 8.2, 8.4, 8.6 shipped** (MCP read+propose tools, in-app Claude Inbox, end-user plugin, 5 skills); 8.3 (DXT bundle) + 8.5 (Agent SDK) remain — see § Phase 8 + [`claude-integration.md`](claude-integration.md) |
...
- [~] **8.5 Embedded Claude agent in Ask Compass** — *backend shipped:* `assistant:agent` runs a bounded **Anthropic tool-use loop** over local data. `llm-client.ts` gained tool-use + **`cache_control` prompt caching** (kept HTTP-only — no SDK, matching the codebase convention). Tools (`assistant-tools.ts`): read `get_upcoming` + `get_finance_summary`, and `propose_task` which enqueues a `pending` `claude_proposals` row (→ Claude Inbox, never a direct write). Vault excluded; finance summaries only; OpenAI keeps the single-shot RAG `ask`. 9 unit tests (tool executor + agent loop + wire contract). *Pending:* renderer UI toggle (agentic vs. RAG) + more tools (notes, habits, txn-tag) + proactive-insights surfacing. *(realizes Phase 7 Track E)*
