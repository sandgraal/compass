...
- `registerPeopleHandlers` — Phase 10.7 "Connect": `people:list` derives a cross-source **people directory** from the people-bearing records (LinkedIn connection/invitation/recommendation/endorsement + Facebook friend — titles that unambiguously name a person, so merchants/newsletters don't leak in), collapsing the SAME person seen through different sources into one entry (touchpoint count + distinct sources + first/last seen) and matching each to `contacts` by normalized display name. Pure aggregator `electron/lib/people.ts` (`extractPersonName` + `buildPeople`); derived on demand, no schema change. Powers the **People** page (`/people`), whose rows deep-link to `/timeline?q=<name>`.
