...
26 tables. Lives at `~/Library/Application Support/Compass/.data/compass.db`.
...
| `finance_accounts` | Bank, credit, investment, debt accounts. Phase 4 added columns: `assetClass` (Phase 4.4 net-worth bucket), `paymentDayOfMonth` (Phase 4.5 forecast), `plaidItemId` + `plaidAccountId` + `mask` (Phase 4.6 Plaid linkage; indexed via `idx_finance_accounts_plaid`), `simplefinConnectionId` + `simplefinAccountId` (Phase 4.7 SimpleFIN linkage; indexed via `idx_finance_accounts_simplefin`), `institution`, `paymentDueDate`. **Phase 11.1** added `currency` (ISO 4217, default `'USD'`) — the account's native denomination, converted to the base currency for net worth. **Phase 11.2** added `is_foreign` (default false; backfilled true for non-USD accounts) — drives FBAR/FATCA aggregation. An account belongs to at most one provider; all provider columns are nullable so manual / CSV / Plaid / SimpleFIN accounts coexist. |
...
| `finance_transactions` | Hashed for dedup. Phase 4.2 promoted `geo` + `purpose` from `notes` tokens to **indexed** columns (`idx_..._geo`, `idx_..._geo_purpose`, `idx_..._geo_date`); Phase 4.3 added the **indexed** `(taxYear, taxTag)` pair for year-end aggregation. Phase 11.1 added `currency` (ISO 4217, default `'USD'`; inherits the owning account's currency). |
...
| `fx_rates` | Phase 11.1 multi-currency. One row per (`date`, `base`→`quote`) exchange rate (`rate` = units of `quote` per 1 `base`). UNIQUE `(date, base, quote)` (`uq_fx_rates_date_base_quote`) makes a re-fetch/re-entry idempotent. The latest row per pair drives net-worth/forecast conversion to the user's base currency; `source` = `manual` (or `erapi` once the daily fetch lands). Migration `0019`. |
...
| `travel_segments` | Phase 11.5 days-in-country — one row per trip OUTSIDE the home country (`country` + inclusive `start_date`/`end_date`). Per-country day counts (home = the year remainder) drive the US substantial-presence test + a CR 183-day check. `source` allows a future calendar/I-94 feed. Migration `0021`. |
| `financial_goals` | Phase 11.6 goals & milestones — target-date savings goals (tax reserve, capex draw, retirement number). `source` resolves the CURRENT value: `manual` (`manual_current`) or an auto-link to a live aggregate (`net-worth` / `retirement` / `property-basis`). `target_amount` + optional `target_date` + planned `monthly_contribution` drive a required-monthly / on-track computation. Base-currency amounts. Migration `0022`. |
| `records` | Phase 10.1 "Acquisition Engine" — the unified append-only **timeline**. Anything the Drop Zone ingests from a data export (Netflix/Spotify history, any dated CSV/JSON) lands here as a typed event; `payload` keeps the original row as JSON; `dedup_hash` UNIQUE (mirrors `finance_transactions.hash`) makes re-imports idempotent. Migration `0016`. Full-text searchable via the external-content **`records_fts`** FTS5 index (title/body/payload, bm25-weighted), kept in sync by AFTER INSERT/UPDATE/DELETE triggers — Phase 10.7 "Converse", migration `0018` (created in `ensureNewTables` too since packaged builds skip migrations; `syncRecordsFts` rebuilds it for rows that predate the index). |
...
- Categories: `financial`, `identity`, `credentials`, `medical`, `legal`, `foreign-accounts` (Phase 11.2 — FBAR/FATCA foreign account numbers + institutions; encrypted at rest, excluded from exports like every category)
...
- `registerFinanceHandlers` — txns, accounts, debt summary, budget, rules, **geo summary, tax summary + override (Phase 4.3), net-worth snapshot/trajectory + capture + manual balance (Phase 4.4), forecast + override CRUD (Phase 4.5), tax-pack export (Phase 5.4: `finance:export-tax-pack`), cleanup tools (Phase 4.7: `finance:merge-accounts` folds a duplicate account into a keeper — reassigns txns, moves the SimpleFIN linkage, deletes the source; `finance:dedupe-transactions` previews/removes same-date+amount+normalized-description duplicates keeping the SimpleFIN copy — both in `electron/integrations/finance-cleanup.ts`), multi-currency (Phase 11.1: `finance:get-currency-settings`, `finance:set-base-currency`, `finance:set-account-currency` (cascades to the account's txns), `finance:get-fx-rates`, `finance:set-fx-rate` (manual upsert), `finance:refresh-fx-rates` (pulls today's rates from `open.er-api.com` — pinned in the CSP `connect-src`, no wildcard; also run by the daily cron at 00:15) — conversion math in `electron/integrations/finance-fx.ts`, the main-process-only fetch in `electron/integrations/finance-fx-fetch.ts`; net worth rolls up to the base currency), CR property / Airbnb P&L (Phase 11.3: `finance:get-property-pnl`, `finance:set-property-config` — pure assembly in `electron/integrations/finance-property.ts` over already-tagged rows (geo/purpose/taxTag), base-currency P&L + cost-basis accumulator + Schedule E straight-line depreciation; config in `app_settings`), expat tax (Phase 11.2: `finance:get-expat-tax-summary`, `finance:set-account-foreign`, `finance:set-fatca-threshold` — FBAR/FATCA max-aggregate-foreign-balance-by-year + foreign-tax-credit ledger in `electron/integrations/finance-expat.ts`, from balances + a `tax:foreign-tax` tag; account IDENTIFIERS live in the encrypted `foreign-accounts` vault category and never flow through the summary or exports), retirement projection (Phase 11.4: `finance:get-retirement-projection`, `finance:set-retirement-config` — pure multi-decade accumulation→decumulation engine in `electron/integrations/finance-retirement.ts`: Social-Security claiming-age math, Airbnb/other income, sequence-of-returns stress path; starting balance auto-sourced from the `retirement`+`savings` net-worth assets, config in `app_settings`; the SSA PIA is manual since the statement recognizer is content-light), days-in-country & residency (Phase 11.5: `finance:get-residency-summary`, `finance:add-travel-segment`, `finance:delete-travel-segment`, `finance:set-residency-config` — pure `electron/integrations/residency.ts` over the `travel_segments` table: per-country day counts (home = the year remainder) → US substantial-presence test + CR 183-day rule + pensionado/rentista/inversionista pathway checklist + CAJA estimate; config in `app_settings`, inversionista investment defaults to the CR property net-worth value), goals & milestones (Phase 11.6: `finance:get-goals-summary`, `finance:add-goal`, `finance:update-goal`, `finance:delete-goal` — pure `electron/integrations/finance-goals.ts` over the `financial_goals` table: per-goal remaining/%/required-monthly/on-track, currents auto-linked to net worth / retirement assets / property cost basis or manual), estate & insurance readiness (Phase 11.7: `finance:get-estate-readiness`, `finance:set-estate-item` — `electron/integrations/finance-estate.ts`: a manual estate-document checklist in `app_settings` + insurance-adequacy/gaps + property surfacing assembled from the `assets` domain. **Reads NO vault data** — it points the user to Vault → Legal for storage)**
...
- `registerRecordsHandlers` — Phase 10 "Acquisition Engine" **Drop Zone**: `records:list` (newest-first, optional `source`/`type` filter + substring `q` over title/body — the browse path; `includeFirehose:false` collapses high-volume/low-signal **firehose** sources — browser history — from the default browse so they don't bury the signal events, non-destructively: the rows stay on disk and selecting the source's chip overrides it. Tiers live in the pure `electron/lib/source-tiers.ts`; `records:stats` returns the `firehose` count for the Timeline's "Show browsing (N hidden)" toggle — Phase 10.7 "Curate"), `records:search` (Phase 10.7 — bm25-ranked FTS5 over title/body/payload via `records_fts`, same `source`/`type`/date filters; the Timeline routes real queries here; `mode:'semantic'` instead runs the opt-in local vector index in `records-embeddings.ts` — one Ollama embedding per record, keyed by id, cosine-ranked — and transparently falls back to FTS keyword when no index is built or Ollama is offline), `records:rebuild-semantic` + `records:semantic-status` (build/extend + report that opt-in index; it auto-extends incrementally after each import, best-effort, only once the user has built it once), `records:on-this-day` (prior-year records sharing today's UTC month/day — also surfaced as the Dashboard **"On this day"** memory card `src/components/OnThisDay.tsx`, grouped by year via the pure `src/lib/on-this-day.ts`, Phase 10.7 "Connect"), `records:stats` (true totals + distinct-source count + dated span for the header), `records:facets` (distinct sources + kinds across the WHOLE table — the Timeline filter chips, so a chip selection pushes server-side through `records:list`'s `source`/`type` filter and spans the full history rather than the loaded 500-row page), `records:import` (file dialog) / `records:import-paths` (drag-drop, paths resolved via `webUtils.getPathForFile` in preload). Recognizers in `electron/lib/recognizers.ts` span five ingestion shapes — text (Netflix·Spotify·YouTube·Amazon·PayPal·Goodreads·Venmo — CSV/JSON; **LinkedIn** `linkedin.ts` covers the full "Basic" export — Connections plus messages (content-light daily counts), positions, certifications, endorsements, invitations, company follows, Learning, events, job applications, and recommendations given/received; **PayPal** filters out internal-mechanics rows (funding legs, auth holds, FX-conversion pairs); preamble-aware via `fromHeaderRow`; **Facebook** + **Google Takeout** in `facebook.ts`/`google.ts` — Google covers every `MyActivity.html` + YouTube watch/search history (typed watch·search·visit·maps·app·assistant), plus structured Takeout files: Chrome `History.json` (→ `browser`/visit), Play Store `Purchase History.json` (→ `google-play`/purchase), Google Pay `transactions_*.csv` (→ `google-pay`/payment), Calendar `.ics` VEVENTs (→ `gcal`/event), Google Fit daily-activity CSVs (→ `google-fit`/fitness, daily rollup), and Google Voice calls/texts (→ `google-voice`, content-light — kind + timestamp from the filename, message text never read); generic dated catch-all LAST), streaming (Apple Health XML, email `.mbox` — `MAX_STREAM_BYTES` 8 GB; the input file is streamed/read incrementally rather than as one in-memory string, so multi-GB Gmail mboxes are feasible), SQLite-file (browser history, iMessage `chat.db`), PDF (credit reports → bureau·score·date, tax documents → form·year, Social Security statements → year, generic document index; text-extracted via `pdf-parse`, **content-light — the sensitive source text is never stored**), and the Google Takeout `.zip` container that recurses entries back through the others — and normalize any export into `records`, deduped by content hash (`onConflictDoNothing`). `updateRecordsKnowledge()` writes a SUMMARY to `timeline/overview.md`; per-record detail isn't mirrored into the knowledge base but IS searchable on demand by the assistant + MCP (Phase 10.7 — `search_records` / `compass_search_timeline`), a scoped opt-in that covers `records` ONLY (the vault + raw finance rows stay aggregates-only). `buildRecordsCsv()` is shared with the Export Center. Local-only; no network, no CSP widening, vault never touched. **Snapshot facts** (Phase 10.x): the same Drop Zone also runs `SNAPSHOT_RECOGNIZERS` (`recognizeSnapshot`) IN ADDITION to the record recognizers — a file can yield both timeline events and static facts; the non-timeline facts go to `snapshot_facts` (deduped via `hashSnapshot`), surfaced by `snapshot:list` (optional `source`/`category` filter) behind themed pages. Recognizers so far: `facebook-ad-profile` (advertisers-with-your-info + targeting categories) → the **Ad Profile** page (`/ad-profile`); `facebook-profile` (the `<th>/<td>` identity table — name/emails/phones/birthday/…) → the **Profile** page (`/profile`); `facebook-apps` (connected + blocked third-party apps) → the **Apps & Websites** page (`/apps`); `google-subscriptions` + `google-bookmarks` (YouTube subscriptions + Chrome bookmarks, category `google-saved`) → the **Saved** page (`/google-saved`).
...
- `registerPeopleHandlers` — Phase 10.7 "Connect": `people:list` derives a cross-source **people directory** from the people-bearing records — the social graph (LinkedIn connection/invitation/recommendation/endorsement + Facebook friend, titles that unambiguously name a person) plus the noisier counterparties (PayPal payees + iMessage/Facebook/LinkedIn message conversation partners) passed through an `isLikelyPerson` person/merchant classifier (drops merchants, newsletters, group threads, phone numbers). Collapses the SAME person across sources into one entry (touchpoint count + distinct sources + first/last seen) and matches each to `contacts` by normalized display name. Pure aggregator `electron/lib/people.ts` (`extractPersonName` + `isLikelyPerson` + `buildPeople`); derived on demand, no schema change. Powers the **People** page (`/people`), whose rows deep-link to `/timeline?q=<name>`.
...
- `registerAssistantHandlers` — Ask Compass (Phase 5.12): `assistant:get-status`, `assistant:set-key`, `assistant:clear-key`, `assistant:set-active-provider`, `assistant:set-model`, `assistant:ask`, `assistant:cancel`, **`assistant:agent` (Phase 8.5)**. The agent is an Anthropic tool-use loop (`electron/integrations/assistant-tools.ts` defines read tools `get_upcoming`/`get_finance_summary`/`get_week_tasks`/`get_weekly_goals`/`get_habit_streaks`/`get_insights`/`get_timeline` — the "plan my week" four added for Phase 7 Track E, `get_timeline` (Phase 10.7) summarizing the unified `records` Timeline by source/kind/year AGGREGATES, and **`search_records`** (Phase 10.7 "Converse") which returns the ACTUAL matching records (capped + char-budgeted, payload never returned) so the agent can answer "what/when did I…" questions — plus a `propose_task` tool that enqueues a `pending` `claude_proposals` row — never a direct write); the Anthropic client (`llm-client.ts`) gained tool-use + `cache_control` prompt caching. Vault excluded; finance summaries only (the `records` timeline is the deliberate, user-opted-in exception — searchable in detail); OpenAI keeps the single-shot `ask` path. BYO Anthropic/OpenAI keys are encrypted via `crypto-vault` primitives at `.vault/assistant.enc`. The raw key never crosses the IPC boundary after being set; the renderer reads only a masked tail through `getStatus`. `assistant:ask` composes Phase 5.9 semantic search (top-K cosine over `.data/knowledge-embeddings.json`) with a keyword-scan fallback when no embedding index exists, then issues a non-streaming HTTP request to the active provider from the main process. CSP-free because main-process fetch isn't constrained.
...
| `/timeline` | `src/pages/Timeline.tsx` (Phase 10 — the Drop Zone + append-only, searchable life Timeline with "on this day" recap and stats header; seeds its search from a `?q=` param so the People page can deep-link "everything involving X") |
| `/people` | `src/pages/People.tsx` (Phase 10.7 "Connect" — the derived people directory; `people:list` collapses the same person across sources and matches your contacts) |
