...
export function writeFinancesDebt(s: FinanceSnapshot): void {
  ...
  const lines = [
    ...
    ...sorted.map(
      (d, i) =>
        `| ${i + 1} | ${d.name} | ${fmtMoney(d.balance)} | ${fmtPct(d.apr, 2)} | ${fmtMoney(d.minPayment)} |`
    ),
    ''
  ]
...
export function writeFinancesMonthly(s: FinanceSnapshot): void {
  ...
  const rows = s.budget.map((b) => {
    ...
    return {
      ...
      pct: b.monthlyAmount ? actual / b.monthlyAmount : 0
    }
  })
  ...
  const lines = [
    ...
    ...rows.map(
      (r) =>
        `| ${r.category} | ${r.subcategory ?? ''} | ${fmtMoney(r.monthlyAmount)} | ${fmtMoney(r.actual)} | ${fmtMoney(r.variance)} | ${fmtPct(r.pct, 0)} |`
    ),
